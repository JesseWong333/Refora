            ...
            <div className="flex flex-wrap gap-1">
              {PROVIDER_TEMPLATES.map((tpl) => (
                <button
                  key={tpl.name}
                  type="button"
                  className="rounded-md border border-border bg-panel-2 px-2 py-1 text-label text-muted hover:border-accent hover:text-foreground"
                  onClick={() => {
                  ...
                  {loadingModels ? (
                    ...
                    filteredModels.map((m) => (
                      <button
                        key={m.id}
                        type="button"
                        className={`flex w-full items-center justify-between gap-2 px-2 py-1.5 text-left text-label hover:bg-hover ${
